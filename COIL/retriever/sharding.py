...
def load_cls_one_split(split_dir: str):
    if os.path.isfile(os.path.join(split_dir, 'cls_reps.pkl')):
        split_cls_reps = np.load(os.path.join(split_dir, 'cls_reps.pkl'), allow_pickle=True)
    # if os.path.isfile(os.path.join(split_dir, 'cls_indices.pkl')):
    #     assert os.path.isfile(os.path.join(split_dir, 'cls_values.pkl')) and os.path.isfile(os.path.join(os.path.join(split_dir, 'cls_shape.pkl'))), 'required files not found!'
    #     cls_indices = np.load(os.path.join(split_dir, 'cls_indices.pkl'), allow_pickle=True)
    #     cls_values = np.load(os.path.join(split_dir, 'cls_values.pkl'), allow_pickle=True)
    #     cls_shape = np.load(os.path.join(split_dir, 'cls_shape.pkl'), allow_pickle=True)
    #     split_cls_reps = sparse.coo_matrix(())
...
def main():
    ...
    for sp in tqdm(splits_in_shard):
        ...
        if cls_reps is None:
            ...
            cls_reps = np.vstack((cls_reps, split_cls_reps), axis=0)
            cls_ids = np.vstack((cls_ids, split_cls_ids), axis=0)

    if args.use_torch:
        cls_reps = cls_reps.tocoo() # convert to numpy coo format
        cls_shape = cls_reps.shape
        cls_indices = torch.LongTensor(np.vstack((cls_reps.row, cls_reps.col)))
        cls_values = torch.FloatTensor(cls_reps.data)

        cls_reps = torch.sparse.FloatTensor(cls_indices, cls_values, torch.Size(cls_shape))
        
        # cls_reps = torch.tensor(cls_reps)
        # cls_reps = cls_reps.to_sparse()
        torch.save(cls_reps, os.path.join(args.save_to, f'shard_{args.shard_id:02d}', 'cls_reps.pt'))
