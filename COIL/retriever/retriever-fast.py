...
def main():
    ...
    parser.add_argument('--sparse_index_vectors', default=True)
    ...
    if args.sparse_index_vectors:
        doc_cls_reps = torch.load(os.path.join(args.doc_shard, 'cls_reps.pt')).to_dense() # .float()
    else:
    ...
    # if args.sparse_vectors:
        # query_cls_reps = torch.load(os.path.join(args.query, 'cls_reps.pt')).to_dense()
    # else:
    query_cls_reps = torch.load(os.path.join(args.query, 'cls_reps.pt')).float()

    print("using device ", query_cls_reps.device)
