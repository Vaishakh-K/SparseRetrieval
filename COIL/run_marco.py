...
from marco_datasets import GroupedMarcoTrainDataset, MarcoPredDataset, MarcoEncodeDataset, MarcoPredictExpansionDataset
from modeling import COIL, COILWithExpansion
...
def main():
    ...
    if training_args.do_encode and os.path.isfile(os.path.join(model_args.model_name_or_path, "model_args.bin")):
        print(f'loading saved model_args from {os.path.join(model_args.model_name_or_path, "model_args.bin")}')
        model_path = model_args.model_name_or_path
        model_args = torch.load(os.path.join(model_args.model_name_or_path, "model_args.bin"))
    ...
    if model_args.do_expansion:
        config.output_hidden_states = True
        model = COILWithExpansion.from_pretrained(
    ...
    if training_args.do_train:
        trainer.train(
            model_path=model_args.model_name_or_path if (os.path.isdir(model_args.model_name_or_path) and training_args.resume_from_checkpoint) else None
        )
        trainer.save_model()
    ...
    if training_args.do_encode:
        ...
        all_cls = sparse.csr_matrix(all_cls)

        with open(os.path.join(data_args.encoded_save_path, f'cls_reps.pkl'), 'wb') as fo:
            pickle.dump(all_cls, fo, protocol= pickle.HIGHEST_PROTOCOL)
    ...
    if training_args.predict_expanison_terms:
        if training_args.local_rank > -1:
            raise NotImplementedError('Encoding with multi processes is not implemented.')
        from torch.utils.data import DataLoader
        encode_dataset = MarcoPredictExpansionDataset(
            data_args.encode_in_path, tokenizer, p_max_len=data_args.p_max_len
        )
        encode_loader = DataLoader(
            encode_dataset,
        ...
        for batch in tqdm(encode_loader):
            with torch.cuda.amp.autocast():
                with torch.no_grad():
                    ...
                    for k, v in batch.items():
                        batch[k] = v.to(training_args.device)
                    expansion_scores, expansion_indices = model.compute_expansion_terms(**batch)
